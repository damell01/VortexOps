<x-filament-widgets::widget>
    ...
    <div class="vx-fc">
        ...
        <div class="flex items-center justify-between gap-3">
            <div><div class="text-sm font-semibold text-gray-950 dark:text-white">Active Show Queue</div><div class="text-xs text-gray-500">Cards show the next operational step; the table below is the detailed view.</div></div>
            <div class="text-xs text-gray-500">{{ number_format($stats['pending_lines']) }} packing lines pending</div>
        </div>

        <div class="vx-fc-grid">
            @forelse($queue as $show)
                @php
                    $stage = $show->getAttribute('fulfillment_stage');
                    $total = max(1, (int)$stage['shipments']);
                    $done = (int)$stage['delivered'];
                    $pct = $stage['shipments'] > 0 ? min(100, round(($done / $total) * 100)) : 0;
                @endphp
                <a class="vx-fc-card" href="{{ \App\Filament\Resources\FulfillmentResource::getUrl('view', ['record' => $show]) }}">
                    ...
                    <div class="mt-3 grid grid-cols-3 gap-2 text-center">
                        <div><div class="vx-fc-label">Packing Lines</div><div class="font-semibold">{{ $stage['orders'] }}</div></div>
                        <div><div class="vx-fc-label">Open Shipments</div><div class="font-semibold">{{ $stage['open'] }}</div></div>
                        <div><div class="vx-fc-label">Assigned</div><div class="truncate font-semibold">{{ $show->fulfillmentUsers->pluck('name')->join(', ') ?: 'None' }}</div></div>
                    </div>
                    @if($stage['shipments'] > 0)
                        <div class="vx-fc-progress" style="color:rgb(5 150 105)"><span style="width:{{ $pct }}%"></span></div>
                        <div class="vx-fc-meta">{{ $stage['delivered'] }} of {{ $stage['shipments'] }} shipments delivered</div>
                    @endif
