...
async function shot(page,name){if(DEBUG)await page.screenshot({path:`/tmp/whatnot-spa-v7-${name}.png`,fullPage:false}).catch(()=>{});}
...
async function ensureTargetRendered(page,target){
  ...
  for(let i=0;i<14;i++){await page.evaluate(()=>window.scrollBy(0,-Math.max(window.innerHeight*2,1200))).catch(()=>{});await page.waitForTimeout(600);if(await page.locator(selector).count().catch(()=>0))return true;}
  return false;
}
...
async function extractShipmentPage(page){
  await page.waitForSelector('tbody[data-testid="shipments-table-body"]',{timeout:15000}).catch(()=>{});
  ...
  const stats=await page.evaluate(()=>{
    const wanted=['Sales','Completed Earnings','Shipping Spend','Items sold','Total Delivered','Pending Delivery']; const out={};
    const strongs=[...document.querySelectorAll('strong')];
    for(const label of wanted){const labelEl=strongs.find(s=>(s.textContent||'').trim()===label);if(!labelEl)continue;const card=labelEl.closest('div.flex.flex-col.p-4.h-24')||labelEl.parentElement?.parentElement;const vals=card?[...card.querySelectorAll('strong')].map(s=>(s.textContent||'').trim()).filter(Boolean):[];out[label]=vals.find(v=>v!==label)||null;}
    return out;
  }).catch(()=>({}));
  const selector='tbody[data-testid="shipments-table-body"] > tr[data-testid^="shipments-"][data-testid$="-row"]';
  const rows=await page.locator(selector).evaluateAll(trs=>trs.slice(0,25).map(tr=>{
    ...
    const trackingLink=td[9]?.querySelector('a[href*="TrackConfirmAction"],a[href*="tracking"],a[target="_blank"]'); const trackingHref=trackingLink?.getAttribute('href')||null;
    const tracking=trackingHref?.match(/[?&]tLabels=([^&]+)/)?.[1]||trackingLink?.textContent?.trim()||null;
    return{shipment_testid:tr.getAttribute('data-testid'),recipient,order_date:text(2)||null,items:text(3)||null,value:text(4)||null,weight:text(5)||null,dimensions:text(6)||null,requirements:text(7)||null,status:text(8)||null,carrier,tracking,tracking_url:trackingHref};
  })).catch(()=>[]);
  return{...(await state(page)),stats,row_count:await page.locator(selector).count().catch(()=>0),rows};
}

async function restorePastTarget(page,target){
  if(!/\/dashboard\/lives(?:[/?#]|$)/.test(page.url())) await clickShows(page);
  await clickPast(page);
  const loaded=await loadPastRows(page,target.live_id);
  const exact=loaded.requested||loaded.rows.find(r=>r.live_id===target.live_id)||target;
  const rendered=await ensureTargetRendered(page,exact);
  return{loaded,exact,rendered};
}
...
(async()=>{
  ...
  try{
    ...
    // Shipments FIRST, while the exact selected Past row is definitely present.
    if(out.target?.shipments_url&&await ensureTargetRendered(page,out.target)){
      ...
      out.stages.shipments_click={found:(await link.count().catch(()=>0))>0,from_url:page.url(),expected_href:out.target.shipments_url};
      if(await link.count().catch(()=>0)){
        await link.click({timeout:8000}).catch(()=>null);
        await page.waitForURL(/\/dashboard\/shipments(?:[/?#]|$)/,{timeout:15000}).catch(()=>{});
        await page.waitForTimeout(7000);
        out.stages.shipments=await extractShipmentPage(page); out.stages.shipments.expected_href=out.target.shipments_url; await shot(page,'04-shipments');
      }
    }

    // Return to the Past list, reload the same selected show, then test Analytics.
    if(out.target?.analytics_url){
      ...
      const restored=await restorePastTarget(page,out.target);
      out.stages.analytics_reload={desired_live_id:out.target.live_id,loaded_count:restored.loaded.rows.length,scroll_passes:restored.loaded.passes,found:!!restored.loaded.requested,rendered:restored.rendered};
      if(restored.rendered){
        const row=page.locator('[data-testid="show-list-item"]').filter({has:page.locator(`a[href="${restored.exact.open_url}"]`)}).first();
        const link=row.locator('a',{hasText:/^See Analytics$/}).first();
        out.stages.analytics_click={found:(await link.count().catch(()=>0))>0,from_url:page.url(),expected_href:restored.exact.analytics_url};
        if(await link.count().catch(()=>0)){
          await link.click({timeout:8000}).catch(()=>null); await page.waitForTimeout(6000); out.stages.analytics=await extractAnalytics(page); out.stages.analytics.expected_href=restored.exact.analytics_url.replace(/&amp;/g,'&'); await shot(page,'05-analytics');
        }
      }
    }
