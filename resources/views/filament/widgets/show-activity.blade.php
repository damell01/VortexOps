...
<x-filament-widgets::widget>
    <section class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900 sm:rounded-2xl sm:p-5">
        <div class="flex items-start justify-between gap-3">
            <div>
                <h2 class="text-sm font-semibold text-gray-950 dark:text-white sm:text-base">Show History</h2>
                <p class="mt-1 max-w-2xl text-xs leading-5 text-gray-500 dark:text-gray-400 sm:text-sm">Whatnot imports, field changes, streamer reports, approvals, and inventory movements in chronological order.</p>
            </div>
            ...
            @forelse($events as $event)
                @php
                    $tone = match($event['type']) {
                        ...
                        'ingestion' => 'bg-cyan-100 text-cyan-700 dark:bg-cyan-950 dark:text-cyan-200',
                ...
                <div class="grid grid-cols-[24px_minmax(0,1fr)] gap-2.5 py-2.5 sm:grid-cols-[28px_minmax(0,1fr)] sm:gap-3 sm:py-3">
                    <div class="mt-0.5 flex h-6 w-6 items-center justify-center rounded-full {{ $tone }} sm:h-7 sm:w-7">
                        ...
                        @elseif($event['type'] === 'ingestion')
                            <x-heroicon-m-arrow-down-tray class="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                    ...
                    <div class="min-w-0 border-b border-gray-100 pb-2.5 last:border-0 dark:border-gray-800 sm:pb-3">
                        <div class="flex flex-col gap-0.5 sm:flex-row sm:items-start sm:justify-between sm:gap-2">
                            ...
                            <div class="shrink-0 text-[10px] text-gray-400 sm:text-xs">{{ $event['at']?->format('M j, Y g:i A') }}</div>
                        </div>
                        <div class="mt-0.5 break-words text-xs leading-5 text-gray-600 dark:text-gray-300 sm:mt-1 sm:text-sm">{{ $event['detail'] }}</div>
                        @if($event['meta'])
                            <div class="mt-0.5 text-[10px] text-gray-400 sm:mt-1 sm:text-xs">{{ $event['meta'] }}</div>
                        @endif
                    </div>
                </div>
            @empty
                <div class="rounded-xl border border-dashed border-gray-300 px-4 py-8 text-center dark:border-gray-700">
                    <div class="text-sm font-medium text-gray-700 dark:text-gray-200">No tracked history yet</div>
                    <p class="mt-1 text-xs text-gray-500">Whatnot imports and show-linked workflow events will appear here.</p>
                </div>
            @endforelse
        </div>
    </section>
