<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            @php $health = $this->stockHealthSummary @endphp
        ...
        {{-- ABC Analysis --}}
        @php $abc = $this->abcAnalysis @endphp
        ...
        <div class="rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 p-6">
            <h3 class="text-sm font-semibold text-red-900 dark:text-red-100 mb-4">🔴 Out of Stock Items ({{ count($this->outOfStockItems) }})</h3>
            <div class="space-y-2">
                @foreach($this->outOfStockItems as $item)
                <div class="flex justify-between items-center p-2 text-sm">
                    <div>
                        <p class="font-medium text-red-900 dark:text-red-100">{{ $item['name'] }}</p>
                        <p class="text-xs text-red-700 dark:text-red-400">SKU: {{ $item['sku'] }}</p>
                    </div>
                    <div class="text-right">
                        <p class="font-semibold text-red-900 dark:text-red-100">${{ number_format($item['avg_cost'], 2) }}</p>
                        <p class="text-xs text-red-700 dark:text-red-400">{{ (int)$item['total_received'] }} total received</p>
                    </div>
                </div>
                @endforeach
            </div>
