...
<div class="space-y-6">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-3">
            <label class="rounded-xl border border-gray-200 p-4 dark:border-gray-700"><span class="text-xs font-semibold text-gray-500">Profit Share Rate</span><div class="mt-2 flex items-center gap-2"><input wire:model.live.debounce.300ms="streamer.payout_percentage" type="number" step=".01" class="w-full rounded-lg border-gray-300 bg-gray-50 text-lg font-bold dark:border-gray-600 dark:bg-gray-800"><span class="font-bold">%</span></div></label>
            <label class="rounded-xl border border-gray-200 p-4 dark:border-gray-700"><span class="text-xs font-semibold text-gray-500">Burden Per Shipment</span><div class="mt-2 flex items-center gap-2"><span class="font-bold">$</span><input wire:model.live.debounce.300ms="streamer_burden_per_shipment" type="number" step=".01" class="w-full rounded-lg border-gray-300 bg-gray-50 text-lg font-bold dark:border-gray-600 dark:bg-gray-800"></div></label>
            <label class="rounded-xl border border-gray-200 p-4 dark:border-gray-700"><span class="text-xs font-semibold text-gray-500">Burden Per Hour</span><div class="mt-2 flex items-center gap-2"><span class="font-bold">$</span><input wire:model.live.debounce.300ms="streamer_burden_per_hour" type="number" step=".01" class="w-full rounded-lg border-gray-300 bg-gray-50 text-lg font-bold dark:border-gray-600 dark:bg-gray-800"></div></label>
        </div>
        <div class="mt-5 rounded-xl border border-primary-200 bg-primary-50/60 p-5 dark:border-primary-900 dark:bg-primary-950/20"><div class="text-xs font-bold uppercase tracking-wider text-primary-600">Current formula</div><div class="mt-3 space-y-2 font-mono text-sm"><div>Product Cost = Σ (Inventory Cost × Quantity Sold)</div><div>Burden = (Shipments × ${{number_format($ship,2)}}) + (Hours Worked × ${{number_format($hour,2)}})</div><div>Net Revenue = Gross Revenue − Product Cost − Burden</div><div class="font-bold text-primary-700 dark:text-primary-300">Streamer Pay = Net Revenue × {{number_format($pct,2)}}%</div></div></div>
        <details class="mt-5 rounded-xl border border-gray-200 p-4 dark:border-gray-700"><summary class="cursor-pointer text-sm font-bold">Advanced: Custom Formula</summary><p class="mt-2 text-xs text-gray-500">Leave blank to use the spreadsheet formula above. Enter a custom formula only when the entire calculation for the default streamer group needs to be different.</p><textarea wire:model="streamer.custom_payout_formula" rows="3" class="mt-3 w-full rounded-lg border-gray-300 bg-gray-50 font-mono text-sm dark:border-gray-600 dark:bg-gray-800" placeholder="Optional custom formula"></textarea></details>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="mt-5 rounded-xl border-2 border-primary-300 p-5 dark:border-primary-800"><div class="flex items-center justify-between"><div><div class="text-xs font-bold uppercase text-primary-600">Customize streamer</div><div class="text-lg font-bold">{{$editingMember?->name}}</div></div><button wire:click="closeMemberEditor" class="text-sm text-gray-500">Close</button></div>
            ...
            <div class="mt-4 flex justify-end gap-3"><button wire:click="resetMemberOverrides({{$editingMember->id}})" class="rounded-lg border px-4 py-2 text-sm font-bold">Reset to Team Formula</button><button wire:click="saveMemberOverrides" class="rounded-lg bg-primary-600 px-4 py-2 text-sm font-bold text-white">Save Override</button></div>
        </div>@endif
    </div>
