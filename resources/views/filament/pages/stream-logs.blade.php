    ...
    <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="text" wire:model.live="searchQuery"
                        placeholder="Stream title or notes…"
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-900 dark:text-white">Stream</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                            <td class="px-4 py-3">
                                ...
                                <div class="text-xs text-gray-500 dark:text-gray-400">ID: {{ $stream->whatnot_show_id ?? $stream->id }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="text-sm text-gray-900 dark:text-white">
                                    @foreach($stream->streamers as $streamer)
                            ...
                            <td class="px-4 py-3 text-center text-sm text-gray-600 dark:text-gray-400">
                                {{ $stream->show_date->format('M d, Y') }}
                                @if($stream->start_time)
                                    <div class="text-xs">{{ $stream->start_time->format('H:i') }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-right text-sm font-semibold text-gray-900 dark:text-white">
                                ${{ number_format($stream->gross_revenue ?? 0, 2) }}
                            </td>
                            <td class="px-4 py-3 text-center text-sm text-gray-600 dark:text-gray-400">
                                {{ count($stream->showOrders) }} items
                            </td>
                            <td class="px-4 py-3 text-center">
                                ...
                                @endphp
                                <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium {{ $statusColors[$stream->status] ?? 'bg-gray-100 text-gray-800' }}">
                                    {{ $statusIcons[$stream->status] ?? '' }}
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                ...
                                <p>No streams found matching your filters</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs text-gray-600 dark:text-gray-400">
                <div>🔴 <strong>Active</strong> — Stream in progress</div>
                <div>✓ <strong>Completed</strong> — Stream finished</div>
