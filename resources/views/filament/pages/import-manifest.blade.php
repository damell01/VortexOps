...
@media(max-width:640px){.vx-ai{gap:10px}.vx-head,.vx-section{padding:14px}.vx-stepper{padding:12px 8px}.vx-step{font-size:9px}.vx-btn{min-height:44px}.vx-footer{display:grid;grid-template-columns:1fr}.vx-footer .vx-btn{width:100%}.vx-source-frame{height:420px}}
</style>
<div class="vx-ai">
    <section class="vx-card vx-head">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div><div class="text-xs font-bold uppercase tracking-wide text-violet-600">AI Manifest</div><h1 class="mt-1 text-2xl font-bold text-gray-950 dark:text-white">{{ $this->record->displayName() }}</h1><p class="mt-1 text-xs text-gray-500">Upload the supplier document, let AI stage the extracted lines, then compare the source and suggestions before final approval.</p></div>
    ...
    @if($stage === 'processing')
        <section wire:poll.10000ms="checkProcessing" class="vx-card vx-section">
            <div class="vx-status"><div class="vx-spin"></div><div><div class="font-bold text-violet-900 dark:text-violet-100">AI analysis is running in the background</div><div class="mt-1 text-xs text-violet-700 dark:text-violet-300">You can close this page or work anywhere else in VortexOps. The source file is retained for review and you will receive a notification when extraction and matching are finished.</div><div class="mt-2 text-[10px] text-violet-500">Task #{{ $aiTaskId }} · Checking status every 10 seconds while this page is open</div></div></div>
    ...
    @if($stage === 'verify')
        <div class="vx-review-layout">
            <section class="vx-card vx-source">
                <div class="vx-source-head">
                    <div class="flex items-start justify-between gap-3">
                        <div><div class="vx-label">Source Document</div><div class="text-sm font-bold">{{ $sourceOriginalName ?: 'Uploaded manifest' }}</div></div>
                        @if($sourceUrl)<a href="{{ $sourceUrl }}" target="_blank" rel="noopener" class="vx-btn">Open Full Size</a>@endif
                    </div>
                </div>
                ...
                @elseif($sourceUrl)
                    <div class="vx-source-fallback"><x-heroicon-o-document-text class="mx-auto h-10 w-10" /><div class="mt-3 text-sm font-semibold">Preview is not available for this file type.</div><div class="mt-1 text-xs">Open the original file while reviewing the extracted lines.</div><a href="{{ $sourceUrl }}" target="_blank" rel="noopener" class="vx-btn primary mt-4">Open Source File</a></div>
                @else
            ...
            <section class="vx-card vx-section space-y-4">
                <div class="flex flex-wrap items-start justify-between gap-3"><div><h2 class="text-sm font-bold">Review AI Suggestions</h2><p class="mt-1 text-xs text-gray-500">The AI job has already staged these manifest lines on the pallet. Review or correct the extracted values and inventory mappings below, then approve them.</p></div><button wire:click="startOver" class="vx-btn">Analyze Another File</button></div>
                <div class="vx-info"><strong>Staged, not received:</strong> approving this review finalizes the manifest data and mappings. It does not mark cases as physically received.</div>
                ...
                <div class="vx-footer border-t border-gray-100 pt-4 dark:border-gray-800"><div class="text-xs text-gray-500">Compare the source document and staged values before approval. AI suggestions remain editable until you approve.</div><button type="button" wire:click="import" wire:loading.attr="disabled" class="vx-btn success"><span wire:loading.remove wire:target="import">Approve Manifest</span><span wire:loading wire:target="import">Saving review…</span></button></div>
            </section>
        </div>
    @endif

    @if($stage === 'done')
        <section class="vx-card vx-section text-center"><x-heroicon-o-check-circle class="mx-auto h-12 w-12 text-emerald-500" /><h2 class="mt-2 text-lg font-bold">Manifest approved</h2><p class="mt-1 text-sm text-gray-500">{{ $created }} lines saved · {{ $matched }} mapped · {{ $unmatched }} unmapped.</p><a href="{{ $palletUrl }}" class="vx-btn primary mt-4">Return to Pallet</a></section>
    @endif
</div>
</x-filament-panels::page>
