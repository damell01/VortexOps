<x-review-layout
    ...
    :breadcrumb="$session->project ? '<a href=\''.route('review.project', $session->project).'\' class=\'text-sm font-medium text-gray-700\'>'.$session->project->name.'</a><span class=\'mx-2 text-gray-300\'>/</span><span class=\'text-sm font-medium text-gray-500\'>'.$session->title.'</span>' : '<span class=\'text-sm font-medium text-gray-500\'>'.$session->title.'</span>'"
>

    <div class="mb-6 flex items-start justify-between">
        <div>
            <a href="{{ $session->project ? route('review.project', $session->project) : route('review.index') }}" class="mb-2 inline-flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600">
        ...
        <div class="space-y-3">
            @foreach ($items as $item)
                @php
                    ...
                    $typeMap = [
                        'annotation' => ['bg-violet-50 text-violet-700', 'Annotation'],
