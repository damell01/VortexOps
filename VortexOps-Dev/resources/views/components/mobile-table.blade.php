@props(['rows' => []])

{{-- Mobile-optimized table component that stacks on small screens --}}

<div class="overflow-x-auto">
    {{-- Desktop view --}}
    <div class="hidden md:block">
        {{ $slot }}
    </div>

    {{-- Mobile view: Card-based layout --}}
    <div class="md:hidden space-y-3">
        @if(count($rows) > 0)
            @foreach($rows as $row)
                <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 shadow-sm">
                    @foreach($row as $label => $value)
                        <div class="flex justify-between items-start mb-3 last:mb-0">
                            <span class="font-medium text-gray-600 dark:text-gray-400 text-sm">{{ $label }}</span>
                            <span class="text-gray-900 dark:text-white text-right ml-2">{{ $value }}</span>
                        </div>
                    @endforeach

                    {{-- Action buttons --}}
                    @if(isset($actions))
                        <div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex gap-2">
                            {{ $actions }}
                        </div>
                    @endif
                </div>
            @endforeach
        @else
            <div class="text-center py-12">
                <svg class="mx-auto h-12 w-12 text-gray-300 dark:text-gray-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                <p class="text-gray-500 dark:text-gray-400">No items found</p>
            </div>
        @endunless
    </div>
</div>
