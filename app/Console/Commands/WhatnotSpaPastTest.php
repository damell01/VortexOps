...
class WhatnotSpaPastTest extends Command
{
    ...
    protected $description = 'Test Past-tab show discovery, analytics, and shipments using a production-like Whatnot browser bootstrap';
    ...
    public function handle(): int
    {
        ...
        $this->line('Path: authenticated Seller Hub → /dashboard/lives → Past → exact row links');
        $this->newLine();

        $process = new Process([
            config('vortex.whatnot.node_bin', 'node'),
            base_path('scripts/whatnot-spa-past-test-v3.cjs'),
            $liveId,
        ], base_path(), $env);
        ...
        $this->newLine();
        $rows = $data['past_rows'] ?? [];
        $this->info('Past rows found: ' . count($rows));
        if ($rows) {
            $table = [];
        ...
        if (! empty($data['target'])) {
            $this->newLine();
            $this->info('Target past-show row:');
            $this->line(json_encode($data['target'], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
        } else {
            $this->warn('Target past show was not found in the currently rendered Past rows.');
        }
