...
<div class="vx-report space-y-4">
 ...
 <div class="vx-kpis">
  ...
  <div class="vx-r-card vx-kpi"><div class="text-xs font-bold text-gray-500">Total Items</div><div class="vx-kpi-value">{{ number_format($summary['items']) }}</div><div class="mt-2 text-xs text-gray-500">Unique SKUs in stock</div></div>
  <div class="vx-r-card vx-kpi"><div class="text-xs font-bold text-gray-500">Total Quantity</div><div class="vx-kpi-value">{{ number_format($summary['quantity']) }}</div><div class="mt-2 text-xs text-gray-500">Units currently in stock</div></div>
 ...
 <div class="vx-r-card overflow-hidden">
  <div class="vx-tabs">
   @foreach(['items'=>'Inventory Items','low-stock'=>'Low Stock','top-value'=>'Top Value Items','overview'=>'Overview'] as $key=>$label)<button wire:click="setTab('{{ $key }}')" class="vx-tab {{ $activeTab===$key?'active':'' }}">{{ $label }}</button>@endforeach
  </div>
  @if($activeTab!=='overview')
  ...
  @else
  <div class="px-4 py-3 text-xs text-gray-500">Choose Inventory Items, Low Stock, or Top Value Items to open the detailed in-app report table.</div>
  @endif
 </div>
</div>
</x-filament-panels::page>