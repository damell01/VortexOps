...
<div class="vx-show">
    ...
    <div class="vx-main">
        ...
        <aside class="space-y-3">
            ...
            <section class="vx-card vx-section"><details><summary class="cursor-pointer list-none"><div class="flex items-center justify-between"><div><h3>Show Details</h3><div class="vx-sub">Secondary show and sync metadata.</div></div><span class="text-xs font-bold text-primary-600">View</span></div></summary><div class="mt-3">@foreach([['Orders',$show->units_sold],['Buyers',$show->buyers_count],['Giveaways',$show->giveaways_count],['Peak viewers',$show->max_concurrent_viewers],['Rating',$show->avg_order_rating],['Last sync',$show->last_synced_at?->format('M j, g:i A')]] as [$label,$value])<div class="vx-fin"><span class="text-gray-500">{{ $label }}</span><strong>{{ $value ?? '—' }}</strong></div>@endforeach</div></details></section>
        </aside>
    </div>
</div>
</x-filament-panels::page>