...
class WhatnotReportingReconciler
{
    ...
    public function reconcileOrders(
        ...
        foreach ($shows->chunk($batchSize) as $chunk) {
            ...
            foreach ($chunk as $show) {
                ...
                if (! $liveId) {
                    $skipped++;
                    $progress && $progress("orders: show #{$show->id} skipped — no Whatnot UUID");
                    continue;
                }
            ...
            foreach ($sources as $source) {
                ...
                if (! is_array($rows) || ! $this->orderBatchLooksPlausible($show, $rows)) {
                    $rejected++;
                    $progress && $progress("orders: show #{$show->id} rejected — implausible row count ".(is_array($rows) ? count($rows) : 0));
                    continue;
                }
    ...
    public function backfillAnalytics(
        ...
        foreach ($shows as $show) {
            ...
            try {
                ...
                $fields = [];
                foreach ([
                    'gross_revenue','whatnot_net','completed_earnings','avg_order_value','giveaway_spend',
                    'units_sold','giveaways_count','buyers_count','first_time_buyers','returning_buyers',
                    'shares_count','max_concurrent_viewers','total_views','show_duration',
                ] as $field) {
    ...
    public function reconcileShipments(
        WhatnotChannel $channel,
        Carbon $since,
        int $batchSize = 25,
        ?callable $progress = null,
    ...
    private function liveId(Show $show): ?string
    {
        foreach ([$show->whatnot_show_id, $show->detail_url] as $value) {
            if (preg_match('/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i', (string) $value, $m)) {
