...
async function shot(page, name) { if (DEBUG) await page.screenshot({ path:`/tmp/whatnot-spa-v5-${name}.png`, fullPage:false }).catch(() => {}); }
async function state(page) {
  const text = await bodyText(page); const title = await page.title().catch(()=> '');
  return { url:page.url(), title, challenged:challenged(text,title), body:text.substring(0,5000) };
}
...
async function loadPastRows(page, desiredLiveId = LIVE_ID) {
  ...
  return {
    ...
    passes,
    requested,
    target: requested || fallback,
    target_source: requested ? 'requested' : (fallback ? 'fallback-current-channel' : null),
  };
}
...
(async()=>{
  ...
  try{
    ...
    out.target_source=loaded.target_source;
    out.stages.past.loaded_count=loaded.rows.length;
    out.stages.past.scroll_passes=loaded.passes;
    ...
    if(out.target?.analytics_url && await ensureTargetRendered(page,out.target)){
      ...
      if(await link.count().catch(()=>0)){
        ...
        out.stages.analytics.expected_href=out.target.analytics_url.replace(/&amp;/g,'&');
